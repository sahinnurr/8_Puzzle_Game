...
public class Board {
   ...
   // An inner method that returns the position of the tile on the board with the given row and column indexes
   private Point getTilePosition(int rowIndex, int columnIndex) {
      // converts the indexes to the positions in StdDraw
      int posX = columnIndex + 1, posY = 3 - rowIndex;
      return new Point(posX, posY);
   }

   // A method to get the current state of the board and returns a 2D array representing the current state of the board.
   public int[][] getBoard() {
      int[][] currentBoard = new int[tiles.length][tiles[0].length];

      for (int i = 0; i < tiles.length; i++) {
         for (int j = 0; j < tiles[i].length; j++) {
            if (tiles[i][j] != null) {
               currentBoard[i][j] = tiles[i][j].number; // Accesses number directly
            }
         }
      }
      return currentBoard;
   ...
   // A method to update the tiles based on the new board configuration
   public void setBoard(int[][] newBoard) {
      for (int row = 0; row < 3; row++) {
         for (int col = 0; col < 3; col++) {
            if (newBoard[row][col] == 0) {
               // Sets the empty cell position
               emptyCellRow = row;
               emptyCellCol = col;
               // Skips setting tile for the empty cell
               tiles[row][col] = null;
            } else {
               // Creates a new tile with the value from the new board configuration
               tiles[row][col] = new Tile(newBoard[row][col]);
            }
         }
      }
   }
